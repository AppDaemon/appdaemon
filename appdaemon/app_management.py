...
class AppManagement:
    def __init__(self, ad: AppDaemon, config):
        ...
        self.AD.services.register_service(
            "appdaemon", "app", "start", self.manage_services
        )
        self.AD.services.register_service(
            "appdaemon", "app", "stop", self.manage_services
        )
        self.AD.services.register_service(
            "appdaemon", "app", "restart", self.manage_services
        )
        self.AD.services.register_service(
            "appdaemon", "app", "reload", self.manage_services
        )
    ...
    async def set_state(self, name, **kwargs):
        ...
        await self.AD.state.set_state(
            "_app_management", "admin", entity_id, _silent=True, **kwargs
        )
    ...
    async def get_state(self, name, **kwargs):
        ...
        return await self.AD.state.get_state(
            "_app_management", "admin", entity_id, **kwargs
        )
    ...
    async def init_admin_stats(self):
        ...
        await self.add_entity(
            self.active_apps_sensor, 0, {"friendly_name": "Active Apps"}
        )
        await self.add_entity(
            self.inactive_apps_sensor, 0, {"friendly_name": "Inactive Apps"}
        )
        await self.add_entity(
            self.total_apps_sensor, 0, {"friendly_name": "Total Apps"}
        )
    ...
    async def initialize_app(self, name):
        if name in self.objects:
            ...
            if init is None:
                self.logger.warning(
                    "Unable to find initialize() function in module %s - skipped", name
                )
        ...
        if term is not None:
            try:
                ...
                error_logger.warning(
                    "Unexpected error running terminate() for %s", name
                )
                error_logger.warning("-" * 60)
                error_logger.warning(traceback.format_exc())
                error_logger.warning("-" * 60)
    ...
    async def start_app(self, app):
        ...
        if (
            "disable" in self.app_config[app]
            and self.app_config[app]["disable"] is True
        ):
            pass
        else:
    ...
    def get_app_debug_level(self, app):
        if app in self.objects:
            return self.AD.logging.get_level_from_int(
                self.objects[app]["object"].logger.getEffectiveLevel()
            )
        else:
            return "None"
    ...
    async def init_object(self, name):
        ...
        if self.get_file_from_module(app_args["module"]) is not None:
            ...
            if "pin_thread" in app_args:
                if (
                    app_args["pin_thread"] < 0
                    or app_args["pin_thread"] >= self.AD.threading.total_threads
                ):
                    self.logger.warning(
    ...
    async def read_config(self):
        ...
        if await utils.run_in_executor(self, os.path.isfile, self.app_config_file):
            ...
            new_config = utils.run_in_executor(
                self.read_config_file, self.app_config_file
            )
        else:
                ...
                if root[-11:] != "__pycache__":
                    for file in files:
                        if file[-5:] == ".yaml" and file[0] != ".":
                            ...
                            config = await utils.run_in_executor(
                                self, self.read_config_file, os.path.join(root, file)
                            )
                            valid_apps = {}
    ...
    async def check_config(self, silent=False, add_threads=True):
        ...
        try:
            latest = await utils.run_in_executor(
                self, self.check_later_app_configs, self.app_config_file_modified
            )
                    ...
                    if name not in self.app_config:
                        ...
                        if "class" in new_config[name] and "module" in new_config[name]:
                            ...
                            await self.add_entity(
                                name,
                                "loaded",
                                {"callbacks": 0, "args": new_config[name]},
                            )
    ...
    def get_active_app_count(self):
        ...
        for name in self.app_config:
            if (
                "disable" in self.app_config[name]
                and self.app_config[name]["disable"] is True
            ):
                pass
            elif name in self.non_apps:
                pass
    ...
    def get_app_from_file(self, file):
        ...
        for app in self.app_config:
            if (
                "module" in self.app_config[app]
                and self.app_config[app]["module"] == module
            ):
                return app
        return None
        ...
        if reload:
            ...
            if app is not None:
                ...
                if self.AD.missing_app_warnings:
                    self.logger.warning(
                        "No app description found for: %s - ignoring", file
                    )
                        ...
                        if file[extlen:] == ext:
                            ...
                            if run is True:
                                ...
                                outfile = utils.rreplace(
                                    infile, ext, filter["output_ext"], 1
                                )
                                command_line = filter["command_line"].replace(
                                    "$1", infile
                                )
    ...
    async def check_app_updates(self, plugin=None, mode="normal"):
        ...
        found_files = []
        modules = []
        for root, subdirs, files in await utils.run_in_executor(
            self, os.walk, self.AD.app_dir, topdown=True
        ):
            # print(root, subdirs, files)
            #
            # Prune dir list
        ...
        if plugin is not None:
            ...
            for app in self.app_config:
                ...
                if "plugin" in self.app_config[app]:
                    for this_plugin in utils.single_or_list(
                        self.app_config[app]["plugin"]
                    ):
        ...
        for mod in modules:
            try:
                ...
                if self.AD.logging.separate_error_log() is True:
                    self.logger.warning(
                        "Unexpected error loading module: %s:", mod["name"]
                    )
    ...
    def get_app_deps_and_prios(self, applist, mode):
            ...
            if "dependencies" in self.app_config[app]:
                for dep in utils.single_or_list(self.app_config[app]["dependencies"]):
                    if dep in self.app_config:
                        ...
                        self.logger.warning(
                            "Unable to find app %s in dependencies for %s", dep, app
                        )
        ...
        try:
            for app in self.topological_sort(deps):
                if "dependencies" in self.app_config[app] or self.app_has_dependents(
                    app
                ):
                    prio_apps[app] = prio
                    prio += float(0.0001)
    ...
    def apps_per_global_module(self, module):
        ...
        for app in self.app_config:
            if "global_dependencies" in self.app_config[app]:
                for gm in utils.single_or_list(
                    self.app_config[app]["global_dependencies"]
                ):
                    if gm == module:
                        apps.append(app)
    ...
    async def manage_services(self, namespace, domain, service, kwargs):
        ...
        if "app" in kwargs:
            ...
            self.logger.warning(
                "App not specified when calling '%s' service. Specify App", service
            )
            return None
