...
class MqttPlugin(PluginBase):
    ...
    def stop(self):
        ...
        if self.mqtt_connected:
            ...
            self.mqtt_client.publish(self.mqtt_will_topic, self.mqtt_shutdown_payload, self.mqtt_qos,
    ...
    def mqtt_on_message(self, client, userdata, msg):
        try:
            ...
            if self.mqtt_wildcards != [] and list(
                    filter(lambda x: x in topic, self.mqtt_wildcards)) != []:  # check if any of the wildcards belong
    ...
    async def get_updates(self):
        ...
        while not self.stopping:
            while (
                    not self.initialized or not already_initialized) and not self.stopping:  # continue until initialization is successful
                if not already_initialized and not already_notified:  # if it had connected before, it need not run this. Run if just trying for the first time
                    try:
                        await asyncio.wait_for(utils.run_in_executor(self, self.start_mqtt_service, first_time_service),
                                               5.0, loop=self.loop)
                        await asyncio.wait_for(self.mqtt_connect_event.wait(), 5.0,
                                               loop=self.loop)  # wait for it to return true for 5 seconds in case still processing connect
                    except asyncio.TimeoutError:
                        self.logger.critical("Could not Complete Connection to Broker, please Ensure Broker at URL %s:%s is correct and broker is not down and restart Appdaemon",
